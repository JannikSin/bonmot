...
4. **Word bank integrity.** `data/en.json` ids are stable slugs; NEVER renumber (user progress is keyed on them). No em or en dashes in any content string, ever (David's hard rule; validator and tests enforce). Definitions are original teaching-voice prose, never dictionary phrasing. Grow the bank via batches + `npm run validate` (dictionary cross-check included).
